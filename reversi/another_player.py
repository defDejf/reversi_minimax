...
class MyPlayer:
    ...
    def move(self, board):
        start = time.time()
        move = self.run_nodes(board, self.my_color, self.depth, time.time() + TIME_LIMIT)
        return move
    ...
    def min_node(self, board, color, depth, end_time, alpha, beta):
        ...
        if moves == [] or (time.time() >= end_time) or depth == 0:
            ...
            for move in moves:
                ...
                new_score = self.max_node(new_board, abs(color-1), depth - 1, end_time, alpha, beta)
    ...
    def max_node(self, board, color, depth, end_time, alpha, beta):
        ...
        if moves == [] or (time.time() >= end_time) or depth == 0:
            ...
            for move in moves:
                new_board = self.update_board(board, move, color)
                new_score = self.min_node(new_board, abs(color-1), depth - 1, end_time, alpha, beta)
    ...
    def run_nodes(self, board, color, depth, end_time):
        ...
        for move in valid_moves:
            new_board = self.update_board(board, move, color)
            score_for_move = self.min_node(new_board, abs(color-1), depth-1, end_time, -999999, 999999)
